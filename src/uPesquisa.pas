...
uses
  ...
  FireDAC.Phys.Intf, uClsCliente, uClsContato;
    ...
    if gblNomeTabela = 'CONTATO' then
      begin
        ...
        if rdg_Ordenar.ItemIndex = 0 then
          ...
          gblCodigo := 'NOME';
        Contatos.AbreTabela(gblNomeTabela,gblCodigo,gblNomepesquisa);
      end;
end;
  ...
  if edtNome.Text = '' then
    ...
    begin
      if gblNomeTabela = 'Cliente' then
        ...
        if gblNomeTabela = 'CONTATO' then
          Contatos.PesquisaTabela(gblNomeTabela, gblCodigo, gblNomepesquisa, Trim(edtNome.Text));
    end;
end;
